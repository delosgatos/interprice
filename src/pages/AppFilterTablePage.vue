...
export default {
    ...
    computed: {
        ...
        filterYears () {
            return this.filters.Currency[this.activeFilters.Currency] ? this.filters.Currency[this.activeFilters.Currency].years : [];
        },
        activeDisplay () {
        ...
        activeYears () {
            return this.activeFilters.Years.sort().map(yi => this.filterYears[yi]);
        },
    },
    watch: {
    ...
    methods: {
        parseDataForFilters () {
            let i = 1;
            this.quotes = this.rawData.Items
                .reduce((prev, curr) => prev.concat((curr.Quote || [ {} ])
                    .map((val) => { 
                        return { ...val, 
                            itemId: curr.Id, 
                            id: i++, 
        ...
        updateFilters () {
            this.filters.Years = this.filterYears;
            this.$nextTick(() => {
        ...
        getDataMethod (payload = {}) {
            const tableData = {
                columns: [ 
                    [
                        { show: true, field: '>', label: ' ', width: 30, sortable: false, rowspan: 2, }, 
                        { show: true, field: 'date', label: 'DATE SENT', width: 80, sortable: true, rowspan: 2, thClass: 'text-left', }, 
                        { show: true, field: 'name', label: 'COMPANY', width: 200, sortable: true, rowspan: 2, thClass: 'text-left', }, 
                    ],
                    [],
                ], 
                rows: this.quotes.filter(val => {
                    val.filtered = (new RegExp( this.activeFilters.Company, "i")).test(val.name);
                    return this.filters.Currency.findIndex((f, i) => this.activeFilters.Currency === i && f.id === val.Currency) !== -1 
                        && this.activeYears.includes(val.Years) 
                        && val.filtered
                        || (!val.hasData && !this.activeFilters.Company || val.filtered)
                }), 
                name: 'InterPrice', 
            };
            this.activeYears.forEach((y) => {
                tableData.columns[0].push({ show: true, field: y, label: y + ' YRS', width: 100, sortable: false, colspan: 2, thClass: 'text-сenter',});
                tableData.columns[1].push({ show: true, group: y, field: 'FIX_' + y, label: 'FIX', width: 50, sortable: false, thClass: 'text-сenter',});
                tableData.columns[1].push({ show: true, group: y, field: 'FRN_' + y, label: 'FRN', width: 50, sortable: false, thClass: 'text-сenter',});
            });
            const minColumns = {};
            const minColumnsIndex = {};
            const sumColumns = {}, averageRow = { 
